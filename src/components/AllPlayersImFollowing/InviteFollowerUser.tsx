...
const InviteFollowerUser = (props) => {
    ...
    const { item, index, like, toggleInviteUser, goToProfile, challengeIsSingleActivity, currentUser, challengesStore, challengeData } = props;
    ...
    useEffect(() => {
        ...
        const unsubscribeToPlayerImFollowing = Firebase.firestore.collection('playerChallenges').doc(`${user.uid}_${challengeData.id}_${challengeData.endDateKey}`).onSnapshot((snap) => {
    ...
    return (
        ...
        >
            ...
            <View paddingL-16 flex row spread paddingR-24 centerV>
                ...
                {playerDoc.active ? <Button outlineColor={Colors.green60} label="Playing" outline color={Colors.green60} size="small" /> : invited ? <Button label="Invited" outline color={Colors.green50} size="small" outlineColor={Colors.green50} /> : <Button label="Invite" onPress={invitePlayer} outline color={Colors.buttonLink} size="small" outlineColor={Colors.buttonLink} />}
            </View>
        </View >
    )
}
