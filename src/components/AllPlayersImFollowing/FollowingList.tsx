...
const FollowingList = inject("smashStore", "challengeArenaStore", "challengesStore")(observer((props) => {
    ...
    const array = [1, 2, 3, 4]
    const { smashStore, goToProfile, challengeIsSingleActivity, challenge, showPlayerSmashes, challengeData, close, challengesStore } = props;
    ...
    const renderFollowingUser = ({ item, index, }) => {
        ...
        if (!item) { return null }
        return (
            <InviteFollowerUser item={item} index={index} {...{ currentUser, toggleInviteUser, challengesStore, like, invite, goToProfile, kFormatter, challengeIsSingleActivity, showPlayerSmashes, challengeData }} />
        );
    }

    const doAsyncStuffInUseEffect = async () => { 

        const unsubscribeToPlayersImFollowing = await Firebase.firestore.collection('users').where('followers', 'array-contains', uid).onSnapshot((snaps) => {
    ...
    useEffect(() => {
        ...
        const unsubscribeToPlayersImFollowing = doAsyncStuffInUseEffect();


        return () => {

            if (unsubscribeToPlayersImFollowing) { unsubscribeToPlayersImFollowing(); }
