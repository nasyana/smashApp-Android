...
> {
   static defaultProps = {
      animations: {
         ...
         transform: [
            { translateY: { inputRange: [0, 1], outputRange: [20, 0] } },
         ],
      },
      delay: 0,
      duration: 300,
   ...
   componentDidMount() {
      const { delay, easing, duration, straight } = this.props;
   ...
   render(): React.Node {
      const { children, animations, straight } = this.props;
      const { animation } = this.state;
            ...
            if (prop !== 'transform') {
               ...
               interpolation.forEach((transformation) => {
                  _.forEach(transformation, (i, key) => {
                     transformStyle.transform.push({
                        [key]: animation.interpolate(i),
                     });
                  });
               });
            }
      ...
      return (
         <Animated.View style={[newStyle, animatedStyle, transformStyle]}>
            {children}
         </Animated.View>
      );
   }
