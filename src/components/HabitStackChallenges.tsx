...
const ActivityChallenges = ({ activity, smashStore, challengesStore, small = false }) => {

    const { myChallengesHash, myChallenges, challengesArray } = challengesStore
    const { habitStacksList } = smashStore;
    const { navigate } = useNavigation();
    const habitStacksInActivity = habitStacksList.filter((c) => c.masterIds.includes(activity.id))
    ...
    const goToChallenge = (challenge) => navigate(Routes.ChallengeArena, { challenge: { id: challenge.challengeId } })
    ...
    return (
        <View style={{ flexWrap: 'wrap', paddingHorizontal: 0, paddingTop: 0 }} row>
            ...
            {habitStacksInActivity?.map((habitStack) => {
                ...
                return <TouchableOpacity style={{ marginRight: 4, backgroundColor: Colors.buttonLink, paddingVertical: small ? 4 : 8, paddingHorizontal: small ? 8 : 12, borderRadius: 16 }}>
                    <Text white R12={small} R14={!small} >{habitStack.name}</Text></TouchableOpacity>
            })}
            {habitStacksInActivity.length === 0 && <Text R14 secondaryContent>Not in any Habit Stacks</Text>}
        </View >
    )
}

export default inject('smashStore', 'challengesStore', 'teamsStore')(observer(ActivityChallenges));