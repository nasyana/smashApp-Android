...
const AddActivity = (props: any) => {
   ...
   const [useInSingleGoals, setUseInSingleGoals] = useState(  editingActivity?.useInSingleGoals || false)
   ...
   const remove = () => {
      Alert.alert('Are you sure?', 'Remove this activity?', [
         ...
         },
         {
            text: 'OK',
            onPress: () =>
            updateDoc(doc(firestore, 'feed', editingActivity.id), { active: false })
  .then(() => {
    smashStore.editingActivity = false;
  })
         },
   ...
   const setActivity = useCallback(
      ({
         ...
         setDoc(doc(firestore, 'feed', finalActivity.id), finalActivity, { merge: true })
         .then(() => {
           smashStore.editingActivity = false;
         });
      },
      [level, hideMulti,activityCategory, hideFromTeams, useInSingleGoals]
   );

   const { control, handleSubmit, register, reset } = useForm({
      defaultValues: {
         text: editingActivity?.text || '',
         useInSingleGoals: editingActivity?.useInSingleGoals || false,
...
{(currentUser.superUser || currentUser.activityManager) && (
               <View flex row spread marginH-16 marginB-16 paddingH-16>
                  ...
                  <Switch
                     ...
                     onColor={Colors.color44}
                     offColor={Colors.color6D}
                     value={useInSingleGoals}
                     onValueChange={setUseInSingleGoals}
                     thumbSize={30}
                  />
               </View>
            )}
