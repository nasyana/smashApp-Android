...
import TodaySmashesTeam from 'modules/Home/components/TodaySmashesTeam';
...
const MeToday = (props) => {
   ...
   useEffect(() => {
      const unsubDailyDoc = onSnapshot(doc(collection(firebaseInstance.firestore, "dailyActivity"), `${team.id}_${todayDateKey}`), (snap) => {
        if (snap.exists()) {
          setDay(snap.data());
        }
      });
   ...
   return (
      <View flex>
         ...
         {true && (
            ...
            <TodaySmashesTeam dayKey={day?.dayKey} day={day} team isMe={true} top={0}/>
               {/* {team && (
                  <MyTodayTeamTarget
                     day={day}
                     SPACING={16}
