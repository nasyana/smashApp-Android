...
const ChallengesBreakdownList = (props) => {
    ...
    const { libraryActivitiesHash, kFormatter } = smashStore;

    const [userChallenges, setUserChallenges] = useState([])
    const gotToChallengeArea = (item) => {

        const formattedChallenge = { id: item.challengeId, name: item.challengeName, targets: item?.targets, picture: item.picture || false }
        console.log('kafs', formattedChallenge)
    ...
    useEffect(() => {
        ...
        const unsubscribeToChallenges = Firebase.firestore.collection('playerChallenges').where('active', '==', true).where('uid', '==', uid).where('endUnix', '>', moment().unix()).onSnapshot((snaps) => {
        ...
        return () => {
            if (unsubscribeToChallenges) { unsubscribeToChallenges() }
        }
    }, [])
    ...
    let DATA = userChallenges.map((playerChallenge) => {
        ...
        const playerChallengeData = challengesStore?.getPlayerChallengeData(playerChallenge) || {};
        ...
        return {
            ...
            selectedGradient: playerChallengeData?.selectedGradient || ['#ccc', '#333'],
            ...playerChallenge,
            playerChallengeData,
    ...
    return (
        <View flex >
            ...
            {DATA.map((item, index) => { 
                return <ActivityDetail item={item} key={index} {...{ libraryActivitiesHash, kFormatter, challengesStore }} gotToChallengeArea={gotToChallengeArea} simple={simple} />;
            })}
