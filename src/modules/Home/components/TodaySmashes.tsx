...
const DOT_SIZE = 4
const TodaySmashes = ({ smashStore, horizontal = false, dayKey = false, day = false, uid }) => {


   const firestore = firebaseInstance?.firestore;
  //  const { uid } = firebaseInstance.auth.currentUser;
   const { todayDateKey, currentUser, currentUserHasPointsEver, libraryActivitiesHash,getCurrentUserMood, getLevelColor,stringLimit } = smashStore;

   const mood = getCurrentUserMood(todayDateKey) || false;
   // const [dailyActivity, setDailyActivity] = useState(todayActivity);
   const [smashes, setSmashes] = useState(!dayKey ? (day?.smashes || smashStore.todayActivity?.smashes || []) : []);
   const goToPost = (id) => {
...
const {navigate} = useNavigation();
   ...
   useEffect(() => {
      const unsubscribe = onSnapshot(
        doc(
          collection(
            doc(collection(firestore, "users"), (uid || currentUser?.uid)),
            "days"
          ),
          dayKey || todayDateKey
        ),
...
if(currentUser.name && !currentUser.allPointsEver){return null}
   return (
      <>
         <View paddingV-16>
            <SectionHeader title={`${headerDate}`} style={{ marginTop: 16 }} />
        ...
        <Text R10 marginL-4>{stringLimit(item.activityName, 10, false)} <Text R10 marginL-4>{moment(item.timestamp, 'X').format('hh:mm')}</Text></Text>
               ...
               {mergedActivities
                  && mergedActivities.map((s,i) => {
                       ...
                       return <SingleActivity goToActivity={true}  day={day} key={s.activityMasterId} activity={activity} smashStore={smashStore} showAllPointsEarned={true} points={s.points || 0}/>;
                    })
                  }
            </View>
