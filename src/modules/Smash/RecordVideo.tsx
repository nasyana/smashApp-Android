...
import { StyleSheet, View, Dimensions, TouchableOpacity, Modal, Platform, Text, Image } from "react-native";
...
@inject("smashStore")
@observer
export default class ShareInModal extends React.Component<ScreenProps<>, ShareState> {
    ...
    async onPressLibrary() {
        ...
        if (!result.cancelled) {
            ...
            if (result.type == 'video') {
                ...
                this.props.smashStore.setCameraVideo({ asset: asset, video: result });
    ...
    async _StartRecord() {
        ...
        if (this.camera) {
            ...
            this.setState({ recording: true }, async () => {
                ...
                // alert('2')
                this.camera.recordAsync({ mute: true })
                    .then((video) => {
    ...
    render(): React.Node {
        ...
        const { hasCameraPermission, type, flashMode, loading, ratio, recording, video } = this.state;
        if (this.props.smashStore.smashModal) { return null }
        ...
        if (!this.props.smashStore.smashModal && this.props.smashStore.cameraModal) {
            return (
               <View
                  style={{
                     position: 'absolute',
